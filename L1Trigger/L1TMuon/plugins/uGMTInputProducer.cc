...
uGMTInputProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  while(!m_endOfBx && !m_filestream.eof()) {
    ...
    if (lineID == "BAR" || lineID == "OVL-" || lineID == "FWD-" || lineID == "OVL+" || lineID == "FWD+") {
      ...
      // if (skip) {
      //   std::cout <<  "skipping" << lineID << " (" << mu.processor() << ") loc phi=" << mu.hwPhi()*0.625 << " CMS phi=" << (globalPhi)*0.625 << std::endl;
      // }
  ...
  // edm::LogDebug("number of barrel muons") << barrelMuons->size() << std::endl;
  // edm::LogDebug("number of ovl muons") << overlapMuons->size() << std::endl;
