...
SelectL1Muons::SelectL1Muons(const edm::ParameterSet& iConfig)
{
  ...
  edm::InputTag ugmt = edm::InputTag("microGMTEmulator");
  consumes<l1t::MuonBxCollection>(ugmt);

  edm::InputTag gmt = edm::InputTag("gtDigis");
  consumes<L1MuGMTReadoutCollection>(gmt);
}
...
SelectL1Muons::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   iEvent.getByLabel("microGMTEmulator", pMuonColl);

   bool haveUGMT = false;

   if (pMuonColl->size(0) > 0) {
    haveUGMT = true;
   }

   Handle<L1MuGMTReadoutCollection> pGMTColl;
   iEvent.getByLabel("gtDigis", pGMTColl);
